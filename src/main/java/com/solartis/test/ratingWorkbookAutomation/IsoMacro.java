...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
	public IsoMacro(String configTableName, String lookupTableName) throws MacroException
	{
		this.configQuery="Select * from "+configTableName;
		this.lookupQuery="Select * from "+lookupTableName;
		configTable = new DatabaseOperation();
		try 
		{
			configTable.GetDataObjects(configQuery);
		}
	...
	public void LoadSampleRatingmodel(String SampleRatingModelPath,LinkedHashMap<String, String> inputData) throws MacroException
	{
		try
		{
			...
			Samplepath= SampleRatingModelPath;
	...
	public void GenerateExpected(LinkedHashMap<String, String> inputData,String Targetpath) throws MacroException
	{
		try
		{
			Targetpath =  Targetpath+inputData.get("testdata")+".xls";
	...
	public void PumpinData(LinkedHashMap<String, String> inputData) throws MacroException
	{
		try
		{
			LinkedHashMap<Integer, LinkedHashMap<String, String>> tablePumpinData = configTable.GetDataObjects(configQuery);
			ExcelOperationsPOI excel=new ExcelOperationsPOI(Targetpath);
			trans= new IsoMacro(configQuery,lookupQuery );
			...
			{		
				...
				if (rowPumpinData.get("flag_for_execution").equalsIgnoreCase("Y"))
				{
					if (rowPumpinData.get("Type").equals("input"))
					{
						...
						{
							excel.write_data(rowNum-1, columnNum, trans.Translation1(Datatowrite, rowPumpinData));
						}
						else
						{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	public void PumpoutData(LinkedHashMap<String, String> outputData,LinkedHashMap<String, String> inputData) throws MacroException    
	{
		...
		{
		ExcelOperationsPOI excel=new ExcelOperationsPOI(Targetpath);
		LinkedHashMap<Integer, LinkedHashMap<String, String>> tablePumpoutData = configTable.GetDataObjects(configQuery);
		excel.refresh();
	...
	protected <T> T Translation1(String Datatowrite, LinkedHashMap<String, String> configTable) throws   MacroException
	{
		T outputdata = null;
		...
		case "Date": 
			...
			break;
		case "Lookup":
			String LookupData = Lookup(Datatowrite, lookupQuery);
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
	protected String  Lookup(String Lookup1,String lookup_query) throws  MacroException
	{
		...
		try 
		{
			LinkedHashMap<Integer, LinkedHashMap<String, String>> tableLookup = Lookup.GetDataObjects(lookup_query);
